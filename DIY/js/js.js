...
var myover=function(el,B){
	...
	if(this.hasClass('common_cloms_btm')){var _C=(el.getCoordinates().height-8)+'px';}
	else{var _C=(C.height-8)+'px';}
	var C=B.options.dragbase.getParent();
	var _flag=false;
	if(C==this.getParent()){
		var D=B.options.dragbase;
		while(D=D.getNext()){
			if(!D)break;if(D==this){_flag=true;break;}
		}
	}
	_flag=this.hasClass('slider')&&_flag;
	if(B.options.dragbase){
		if(B.options.basecount>0){B.options.dragbase.getNext().remove()}
		else{el.getNext().remove()}
		B.options.dragbase.remove();
	}

	(B.options.basecount)++;
	if(_flag){
		B.options.dragbase=new Element('div',{'class':'highspan'});
		B.options.dragbase.injectAfter(this);
		new Element('div',{'class':'highspan'}).injectAfter(this);
	}else{
		B.options.dragbase=new Element('div',{'class':'highspan'});
		B.options.dragbase.injectBefore(this);
		new Element('div',{'class':'highspan'}).injectBefore(this);
	}
}
...
function DragStart(){
	...
	var allDrag=[];
	_drag.each(function(el){
		allDrag.include(new MyDrag(el.getParent().addEvent('emptydrop',myemptydrop),{handle:el,droppables:$ES('.common_cloms_mod_drag','div').extend($ES('.common_cloms_btm','div')).remove(el.getParent())}));
	})
}
function showGrid(obj){
	var GridType = obj.id;
	if(!CommonMod.Grid[GridType]){return false;};
...
function isHideOp(event){
	...
	if ((target.className !== 'common_cloms_btm') && (target.className !== 'AddNewModBtn')){
		hideOprator();
		if(target.id == 'common_grids'){$('common_grids').setStyle('display','block');}
		if(target.id == 'AllMod'){$('AllMod').setStyle('display','block');}
	}
}
